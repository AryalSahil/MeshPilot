...
export function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    setSubmitting(true);
    try {
      // Simulate continuous authorization
      await new Promise(resolve => setTimeout(resolve, 850));
      // Register or fetch user
      await login('guest@meshpilot.io', 'MeshPilotPass123!');
      navigate('/dashboard');
    } catch (err) {
      // If user isn't created yet, sign up first then log in
      try {
        const { signup } = useAuth();
        await signup('Demo Pilot', 'guest@meshpilot.io', 'MeshPilotPass123!');
        navigate('/dashboard');
      } catch (e: any) {
        setError('Google login simulation failed. Please try traditional signup.');
      }
    } finally {
      setSubmitting(false);
    }
